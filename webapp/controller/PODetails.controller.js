sap.ui.define([
	...
	"sap/m/MessageBox",
	"com/ibs/ibsappivenpcanalytics/model/formatter",
	...
	return Controller.extend("com.ibs.ibsappivenpcanalytics.controller.PODetails", {
		formatter: formatter,
		onInit: function () {
            debugger
		...
		handleRouteMatched: function (oEvent) {
			...
			com.ibs.ibsappivenpcanalytics.ind2 = "X";
		...
		showServiceItem: function (oEvent) {
			...
			if (!this.ViewSPO) {
				this.ViewSPO = sap.ui.xmlfragment(
					"com.ibs.ibsappivenpcanalytics.view.fragments.servicePO", this
				);
			...
			if (selectKey === "03") {
				...
				if (!this.ViewCPO) {
					this.ViewCPO = sap.ui.xmlfragment(
						"com.ibs.ibsappivenpcanalytics.view.fragments.changePO", this
					);
		...
		showchngPoItem: function (oEvent) {
			...
			if (!this.eViewCPO) {
				this.eViewCPO = sap.ui.xmlfragment(
					"com.ibs.ibsappivenpcanalytics.view.fragments.EditChngPO", this
				);
		...
		onSubmitPress: function () {
			...
			if (!context._saveFragment) {
				context._saveFragment = sap.ui.xmlfragment(
					"com.ibs.ibsappivenpcanalytics.view.fragments.saveChange",
					context
				);
				...
				if (data[i].code === "POC/001") {
					...
					if (!this.eAction) {
						this.eAction = sap.ui.xmlfragment(
							"com.ibs.ibsappivenpcanalytics.view.fragments.errorHandle",
							this
						);
